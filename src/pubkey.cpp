...
// RSA 鍵の生成
	...
	for (int i=0; i < MAX_KEY; i++) {
		...
		if (i == KEY_2048) {
			...
			HCRYPTHASH	hHash;
			if (::CryptCreateHash(cfg->priv[i].hCsp, CALG_SHA_256, 0, 0, &hHash)) {
				tmplen = 512;
				if (::CryptHashData(hHash, tmp, 1, 0)) {
					if (::CryptSignHash(hHash, AT_KEYEXCHANGE, 0, 0, 0, &tmplen)) {
						if (::CryptSignHash(hHash, AT_KEYEXCHANGE, 0, 0, tmp+512, &tmplen)) {
							sign_capa = TRUE;
						}
						else GetLastErrorMsg(Fmt("CryptSignHash2=%d", tmplen));
					}
					else GetLastErrorMsg("CryptSignHash");
				}
				else GetLastErrorMsg("CryptHashData");
				::CryptDestroyHash(hHash);
			}
			else GetLastErrorMsg("CryptCreateHash");

			if (!sign_capa) {
				cfg->pub[i].SetCapa(cfg->pub[i].Capa() & ~(IPMSG_SIGN_SHA1|IPMSG_SIGN_SHA256));
			}
		}
		if (ret) {
			ret = FALSE;
...
BOOL MakeDefaultRSAKey()
{
	...
	const char	*csp_name = IsWinVista() ? MS_ENH_RSA_AES_PROV : MS_ENH_RSA_AES_PROV_XP;
	BOOL		ret = FALSE;

	for (int i=0; flags[i]; i++) {
		HCRYPTPROV	hCsp = NULL;
		if (::CryptAcquireContext(&hCsp, NULL, csp_name, PROV_RSA_AES, flags[i])) {
...
// rebuld 時には、事前に公開鍵を消去
	if ((ctl_flg & KEY_REBUILD) /* && pubKey.Key() == NULL */ ) {
		...
		if (!::CryptAcquireContext(&hCsp, contName, csp_name, PROV_RSA_AES,
				CRYPT_DELETEKEYSET|CRYPT_MACHINE_KEYSET)) {
			if (ctl_flg & KEY_DIAG) {
				GetLastErrorMsg("CryptAcquireContext(destroy)");
			}
		}
		::CryptAcquireContext(&hCsp, contName, csp_name, PROV_RSA_AES, CRYPT_DELETEKEYSET);
		pubKey.UnSet();
		privKey.UnSet();
	}
...
// open key cotainer
	for (int i=0; AcqFlgs[i] != -1; i++) {
		hCsp = NULL;
		hPubCsp = NULL;
		if (::CryptAcquireContext(&hCsp, contName, csp_name, PROV_RSA_AES, AcqFlgs[i])) {
			::CryptAcquireContext(&hPubCsp, NULL, csp_name, PROV_RSA_AES, CRYPT_VERIFYCONTEXT);
			break;
		}
	}
	if (hCsp == NULL) {
...
// プライベート鍵をimport (1024/2048bit)
	if (cfg->priv[kt].blob) {
		if (LoadPrivBlob(&cfg->priv[kt], data, &len)
			...
			::CryptAcquireContext(&hCsp, contName, csp_name, PROV_RSA_AES, CRYPT_MACHINE_KEYSET);
			::CryptAcquireContext(&hPubCsp, NULL, csp_name, PROV_RSA_AES, CRYPT_VERIFYCONTEXT);
		}
		if (hPrivKey == NULL) {
