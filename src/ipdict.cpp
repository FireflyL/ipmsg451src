...
bool IPDict::put_int_list(const char *key, const IPDictIntList &val)
{
	...
	for (auto itr=val.begin(); itr != val.end(); itr++) {
		DynBuf	*dbuf = new DynBuf(ipdict_pack_int(*itr, 0, 0) + 1);
		size_t	len = ipdict_pack_int(*itr, dbuf->Buf(), dbuf->Size());
...
bool IPDict::put_str_list(const char *key, const IPDictStrList &val)
{
	...
	for (auto itr=val.begin(); itr != val.end(); itr++) {
		DynBuf	*dbuf = new DynBuf(ipdict_pack_str(0, (*itr)->Len(), 0, 0) + 1);
		size_t	len = ipdict_pack_str((*itr)->s(), (*itr)->Len(), dbuf->Buf(), dbuf->Size());
		dbuf->SetUsedSize(len);
		vlist.push_back(shared_ptr<DynBuf>(dbuf));
		content_len += len + 2 + len_to_hexlen(len);
	}
...
bool IPDict::put_bytes_list(const char *key, const IPDictBufList &val)
{
	...
	for (auto itr=val.begin(); itr != val.end(); itr++) {
		DynBuf	*dbuf = new DynBuf(ipdict_pack_bytes(0, (*itr)->UsedSize(), 0, 0) + 1);
		size_t	len = ipdict_pack_bytes((*itr)->Buf(), (*itr)->UsedSize(), dbuf->Buf(), dbuf->Size());
		dbuf->SetUsedSize(len);
		vlist.push_back(shared_ptr<DynBuf>(dbuf));
		content_len += len + 2 + len_to_hexlen(len);
	}
...
bool IPDict::put_bytes_str_list(const char *key, const IPDictBufList &val)
{
	...
	for (auto itr=val.begin(); itr != val.end(); itr++) {
		DynBuf	*dbuf = new DynBuf(ipdict_pack_bytes_str(0, (*itr)->UsedSize(), 0, 0) + 1);
		size_t	len = ipdict_pack_bytes_str((*itr)->Buf(), (*itr)->UsedSize(), dbuf->Buf(), dbuf->Size());
		dbuf->SetUsedSize(len);
		vlist.push_back(shared_ptr<DynBuf>(dbuf));
		content_len += len + 2 + len_to_hexlen(len);
	}
...
bool IPDict::put_dict_list(const char *key, const IPDictList &val)
{
	...
	for (auto itr=val.begin(); itr != val.end(); itr++) {
		DynBuf	*dbuf = new DynBuf((*itr)->content_size() + 1);
		size_t	len = (*itr)->pack_content(dbuf->Buf());
		dbuf->SetUsedSize(len);
		vlist.push_back(shared_ptr<DynBuf>(dbuf));
		content_len += len + 2 + len_to_hexlen(len);
	}
...
bool IPDict::put_ipdict_list(const char *key, const IPDictList &val)
{
	...
	for (auto itr=val.begin(); itr != val.end(); itr++) {
		DynBuf	*dbuf = new DynBuf((*itr)->pack_size() + 1);
		size_t	len = (*itr)->pack(dbuf->Buf(), dbuf->Size());
		dbuf->SetUsedSize(len);
		vlist.push_back(shared_ptr<DynBuf>(dbuf));
		content_len += len + 2 + len_to_hexlen(len);
	}
...
bool IPDict::get_int_list(const char *key, IPDictIntList *val) const
{
	...
	for (auto itr=vlist.begin(); itr != vlist.end(); itr++) {
		int64	v;
		bool ret = ipdict_parse_int((*itr)->Buf(), (*itr)->UsedSize(), &v);
		if (!ret) return false;
		val->push_back(v);
	}
	return	true;
...
bool IPDict::get_str_list(const char *key, IPDictStrList *val) const
{
	...
	val->clear();
	for (auto itr=vlist.begin(); itr != vlist.end(); itr++) {
		size_t	len = (*itr)->UsedSize() + 1;
		U8str	*u8 = new U8str((int)len);

		if ((*itr)->UsedSize() > 0) {
			len = ipdict_parse_str((*itr)->Buf(), (*itr)->UsedSize(), u8->Buf(), len);
			if (len == 0) return false;
		} else {
			len = 0;
		}
...
bool IPDict::get_bytes_list(const char *key, IPDictBufList *val) const
{
	...
	val->clear();
	for (auto itr=vlist.begin(); itr != vlist.end(); itr++) {
		size_t	len = (*itr)->UsedSize();
		DynBuf	*dbuf = new DynBuf(len);

		if ((*itr)->UsedSize() > 0) {
			len = ipdict_parse_bytes((*itr)->Buf(), (*itr)->UsedSize(), dbuf->Buf(), len);
			if (len == 0) return false;
		} else {
			len = 0;
		}
...
bool IPDict::get_bytes_str_list(const char *key, IPDictBufList *val) const
{
	...
	val->clear();
	for (auto itr=vlist.begin(); itr != vlist.end(); itr++) {
		size_t	len = b64_to_len((*itr)->UsedSize()) + 1; // ipdict_parse_bytes(s,len,0,0)
		DynBuf	*dbuf = new DynBuf(len);

		if ((*itr)->UsedSize() > 0) {
			len = ipdict_parse_bytes_str((*itr)->Buf(), (*itr)->UsedSize(), dbuf->Buf(), len);
			if (len == 0) return false;
		} else {
			len = 0;
		}
...
bool IPDict::get_dict_list(const char *key, IPDictList *val) const
{
	...
	for (auto itr=vlist.begin(); itr != vlist.end(); itr++) {
		IPDict	*d = new IPDict();
		if ((*itr)->UsedSize() > 0) {
			size_t size = d->unpack_core((*itr)->Buf(), (*itr)->UsedSize());
			if (size == 0) return false;
			if (size != (*itr)->UsedSize()) {
				Debug("internal size mismatch %zd %zd\n", size, (*itr)->UsedSize());
			}
		}
		val->push_back(shared_ptr<IPDict>(d));
	}
	return	true;
}
...
bool IPDict::get_ipdict_list(const char *key, IPDictList *val) const
{
	...
	for (auto itr=vlist.begin(); itr != vlist.end(); itr++) {
		IPDict	*d = new IPDict();
		size_t size = d->unpack((*itr)->Buf(), (*itr)->UsedSize());
		if (size == 0) return false;
		if (size != (*itr)->UsedSize()) {
