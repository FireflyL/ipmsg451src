	...
	const std::vector<HostSub> *recvList, int64 *msg_id)
{
	...
	if (recvList && recvList->size() >= 1) {
		for (auto itr=recvList->begin(); itr != recvList->end(); itr++) {
			p = buf;
			p += strcpyz(p, LOGMSG_CC);
			p += MakeListString(cfg, (HostSub *)&(*itr), hosts, p, TRUE);
			p += strcpyz(p, "\r\n");
			Write(buf);

			SetLogMsgUser(cfg, (HostSub *)&(*itr), hosts, logMsg);
		}
	}
...
BOOL LogMng::ReadCheckStatus(MsgIdent *mi, BOOL is_recv, BOOL unread_tmp)
{
	...
	for (auto itr=msg.host.begin(); itr != msg.host.end(); itr++) {
		if (mi->uid != itr->uid || mi->host != itr->host) {
			continue;
		}
		if (itr->flags & DB_FLAGMH_UNOPEN) {
			itr->flags &= ~DB_FLAGMH_UNOPEN;
			logDb->UpdateOneMsgHost(msg_id, &(*itr));
...
int LogMng::MakeMsgHead(LogMsg *msg, WCHAR *wbuf, int max_len, BOOL with_self)
{
	...
	if (msg->clip.size() || msg->files.size()) {
		...
		for (auto itr=msg->files.begin(); itr != msg->files.end(); itr++) {
			len += wcsncpyz(wbuf + len, L" ", max_len - len);
			len += wcsncpyz(wbuf + len, itr->s(), max_len - len);
		}
		for (auto itr=msg->clip.begin(); itr != msg->clip.end(); itr++) {
			len += wcsncpyz(wbuf + len, L" ", max_len - len);
			len += wcsncpyz(wbuf + len, itr->fname.s(), max_len - len);
		}
		len += wcsncpyz(wbuf + len, L"\r\n", max_len - len);
	}
