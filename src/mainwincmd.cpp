...
BOOL TMainWin::CmdSendMsg(const IPDict &in, IPDict *out, HWND targ_wnd)
{
	...
	if (ipt_list.size() == 1 && **(ipt_list.begin()) == "ALL") {
		...
		for (auto itr=ipt_list.begin(); itr != ipt_list.end(); itr++) {
			Host *host = FindHostForCmd(*itr);
			if (!host) {
				out->put_str(IPMSG_ERRINFO_KEY, Fmt("user not found(%s)", (*itr)->s()));
				return FALSE;
			}
	...
	if (in.get_str_list(IPMSG_FILELIST_KEY, &ipf_list)) {
		for (auto itr=ipf_list.begin(); itr != ipf_list.end(); itr++) {
			if (GetFileAttributesU8((*itr)->s()) == 0xffffffff) {
				out->put_str(IPMSG_ERRINFO_KEY, Fmt("file not found %s", (*itr)->s()));
				return FALSE;
			}
			f_vec.push_back(*itr);
		}
...
{
	recvHwndVec.push_back(targ_wnd);
	return	TRUE;
}
...
BOOL TMainWin::CmdGetState(const IPDict &in, IPDict *out)
{
	...
	len += snprintfz(buf, MAX_UDPBUF, "IP Messenger for Win ver%s\n", GetVersionStr());
