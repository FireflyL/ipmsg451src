static char *mainwin_id = 
	"@(#)Copyright (C) H.Shirouzu 1996-2017   mainwin.cpp	Ver4.50";
/* ========================================================================
...
TMainWin::TMainWin(Param *_param, TWin *_parent) : TWin(_parent)
{
	...
	InstallExceptionFilter(title, LoadStr(IDS_EXCEPTIONLOG), exception_log);
...
void TMainWin::InitToastDll(void)
{
	...
	WCHAR	dir[MAX_PATH_U8];

	GetModuleFileNameW(NULL, path, wsizeof(path));
	GetParentDirW(path, dir);
	MakePathW(path, dir, TOASTDLL);

	if ((hToast = ::LoadLibraryW(path))) {
		...
		if (_ToastMgrInit && _ToastShow && _ToastHide && _ToastIsAlive) {
			if (_ToastMgrInit(hWnd, WM_NOTIFY_TRAY)) {
				MakePathW(path, dir, IPMSG_PNG);
				ipmsgPng = wcsdup(path);
				MakePathW(path, dir, IPEXC_PNG);
...
BOOL TMainWin::CleanupProc()
{
	...
	if (cfg->useLockName) {
		BOOL	lock = ::GetForegroundWindow() ? FALSE : TRUE;
		BOOL	need_br = FALSE;
		...
		if (lock && desktopLockCnt < DESKTOP_LOCKMAX) {
			...
			need_br = desktopLockCnt >= DESKTOP_LOCKMAX ? TRUE : FALSE;
			desktopLockCnt = 0;
		}
		if (need_br) {
	...
	const char *auto_saved)
{
	...
	if (recvHwndVec.size() > 0) {
		...
		for (auto itr=recvHwndVec.begin(); itr != recvHwndVec.end(); itr++) {
			HWND	targ_wnd = *itr;
			if (!::IsWindow(targ_wnd)) continue;
	...
	BOOL force_icon)
{
	...
	if (msg) {
		if (trayMode != _tray_mode && trayMode != TRAY_NORMAL) {
			KillTimer(trayMode == TRAY_RECV ? IPMSG_BALLOON_RECV_TIMER : IPMSG_BALLOON_OPEN_TIMER);
		}
		trayMode = _tray_mode;
	...
	if (hToast) {
		if (msg) {
			ret = _ToastShow(U8toWs(title), U8toWs(msg),
				trayMode == TRAY_RECV ? ipmsgPng : ipexcPng);
		}
		else {
			ret = _ToastHide();
		}
	...
	if (timer) {
		if (IsWin10()) {
			SetTimer(IPMSG_BALLOON_RESET_TIMER, 100);
		} else {
...
void TMainWin::Popup(UINT resId)
{
	...
	if (info.packetCnt) {
		len = strcpyz(buf, LoadStrU8(IDS_DOWNLOAD));
	...
	// 未開封モニタ
	len += strncpyz(buf + len, LoadStrU8(histDlg->UnOpenedNum() ? IDS_UNOPENED : IDS_OPENED)
		, sizeof(buf) - len);
	if (histDlg->UnOpenedNum()) {
		len += snprintfz(buf + len, sizeof(buf) - len, "(%d)", histDlg->UnOpenedNum());
	}
...
char *TMainWin::GetNickNameEx(void)
{
	...
	else if (desktopLockCnt >= DESKTOP_LOCKMAX && cfg->useLockName && *cfg->LockName) {
		snprintfz(buf, sizeof(buf), "%s [%s]", *cfg->NickNameStr ?
									cfg->NickNameStr : msgMng->GetOrgLocalHost()->u.userName,
									cfg->LockName);
	}
