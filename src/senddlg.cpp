static char *senddlg_id = 
	"@(#)Copyright (C) H.Shirouzu 1996-2017   senddlg.cpp	Ver4.50";
/* ========================================================================
...
TSendDlg::TSendDlg(MsgMng *_msgmng, ShareMng *_shareMng, THosts *_hosts, Cfg *_cfg,
	...
	: TListDlg(SEND_DIALOG, _parent), editSub(_cfg, this), hostListView(this),
		hostListHeader(&hostListView), imageWin(_cfg, this), sendBtn(this),
		captureBtn(this), refreshBtn(this), menuCheck(this),
		memCntText(this), repFilCheck(this)
{
...
//	hCurMenu		= NULL;
	...
	if (rInfo) {
		...
		if (rInfo->fileList) {
			...
			for_each (rInfo->fileList->begin(), rInfo->fileList->end(), [&](auto &u) {
...
BOOL TSendDlg::EvCreate(LPARAM lParam)
{
	...
	for (auto itr=replyList.begin(); itr != replyList.end(); itr++) {
		if (Host *host = cfg->priorityHosts.GetHostByName(&(*itr))) {
			if (host->priority <= 0) {
				hiddenDisp = TRUE;
				break;
			}
...
#define UNAMELIST_BUF	((MAX_ULIST + 1) * (MAX_LISTBUF + 100))
	...
	*wu = 0;
	for (auto itr=replyList.begin(); itr != replyList.end(); itr++) {
		...
		MakeListString(cfg, &(*itr), hosts, buf);
...
BOOL TSendDlg::IsReplyListConsist()
{
	...
	for (int i=0; i < memberCnt && cnt > 0; i++) {
		if (hostListView.IsSelected(i)) {
			BOOL	is_same = FALSE;
			for (auto itr=replyList.begin(); itr != replyList.end(); itr++) {
				if (IsSameHost(&(*itr), &hostArray[i]->hostSub)) {
					is_same = TRUE;
					cnt--;
					break;
				}
...
BOOL TSendDlg::IsFilterHost(Host *host)
{
	...
	if (repFilDisp) {
		BOOL	is_match = FALSE;
		for (auto itr=replyList.begin(); itr != replyList.end(); itr++) {
			if (IsSameHost(&(*itr), &host->hostSub)) {
				is_match = TRUE;
				break;
			}
		}
...
BOOL TSendDlg::MakeMsgPacket(SendEntry *entry)
{
	...
	if (entry->Host()->hostStatus & IPMSG_CAPIPDICTOPT) {
		IPDict	dict;
