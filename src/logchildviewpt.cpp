static char *logchildviewpt_id = 
	"@(#)Copyright (C) H.Shirouzu and Asahi Net, Inc. 2015-2017   logchildviewpt.cpp	Ver4.50";
/* ========================================================================
...
BOOL TChildView::PaintPushBox(TRect rc, BoxType type, BOOL is_direct)
{
	HDC 	hDc = hMemDc;
	...
	::SelectObject(hDc,
		...
		type == SELECT_BOX ? hShainPen  : hGrayPen);
...
int TChildView::PaintFromTo(ViewMsg *msg, int *_off_x, int top_y, BOOL is_rev)
{
	...
	HGDIOBJ	old = ::SelectObject(hTmpDc, (!is_rev && lastReplyBtnId == msg->msg.msg_id)
		? hReplyMsgBmp : is_from ? hFromMsgBmp : (is_rev ? hToMsg2Bmp : hToMsgBmp));

	::BitBlt(hMemDc, irc.x(), irc.y(), irc.cx(), irc.cy(), hTmpDc, 0, 0, SRCCOPY);

	::SelectObject(hTmpDc, old);
...
int TChildView::PaintUsers(ViewMsg *msg, int top_y, int *_off_x, int lim_x)
{
	...
	for (i=0; i < msg->msg.host.size(); i++) {
		...
		rc.Init(off_x + headDiff2, utop_y + OffPix(headCy, btn_cy), sz.cx + headDiff2, btn_cy);
		need_crop = (rc.left + sz.cx/3 + unopen_cx + CROPBTN_SIZE * (usr_remain ? 2 : 1))
			>= lim_x ? TRUE : FALSE;

		if (need_crop) {
			HGDIOBJ	old = ::SelectObject(hTmpDc, hCropUserBmp);

			::BitBlt(hMemDc, rc.left, utop_y + OffPix(headCy, CROPUSER_CY),
				CROPUSER_CX, CROPUSER_CY, hTmpDc, 0, 0, SRCCOPY);
			::SelectObject(hTmpDc, old);
			...
			TRect	rc2(rc.x(), utop_y, rc.cx(), headCy + (fontLdCy ? fontLdCy/3 : 1));
			DrawTextW(hMemDc, nick.Buf(), nick.Len(), &rc2,
				(reduce >= 0 ? DT_CENTER : 0)|DT_VCENTER|DT_SINGLELINE|DT_NOPREFIX);

			if (is_unopen) {
				TRect	urc(rc.right + MINI_GAP, rc.top, UNOPEN_CX, UNOPEN_CY);
				HGDIOBJ	old = ::SelectObject(hTmpDc, is_from ? hUnOpenRBmp : hUnOpenSBmp);
				::BitBlt(hMemDc, urc.left, urc.top, urc.cx(), urc.cy(), hTmpDc, 0, 0, SRCCOPY);
				::SelectObject(hTmpDc, old);
		...
		rc.left -= 1;
	//	rc.top  -= 1;
		PaintPushBox(rc, is_selected ? SELECT_BOX : NORMAL_BOX);
	...
	if (is_from && !need_crop && (msg->msg.flags & DB_FLAG_MULTI) && msg->msg.host.size() == 1) {
		...
		HGDIOBJ	old = ::SelectObject(hTmpDc, hCropUserBmp);
		::BitBlt(hMemDc, off_x + MINI_GAP, utop_y + OffPix(headCy, CROPUSER_CY),
			CROPUSER_CX, CROPUSER_CY, hTmpDc, 0, 0, SRCCOPY);
		::SelectObject(hTmpDc, old);
...
//	TRect	rc_box(off_x, top_y + 1, diff_x, headCy - 2);
//	PaintRect(hMemDc, hGrayPen, hWhiteBrush, rc_box, 5, 5);
...
int TChildView::PaintFileInHead(ViewMsg *msg, int top_y, int off_x, int *_lim_x)
{
	...
	// アイコン描画
	HGDIOBJ	oldBmp = ::SelectObject(hTmpDc, hFileBmp);
	::BitBlt(hMemDc, rc.x(), rc.y(), FILEBMP_CX, FILEBMP_CY, hTmpDc, 0, 0, SRCCOPY);
...
int TChildView::PaintClipInHead(ViewMsg *msg, int top_y, int off_x, int *_lim_x)
{
	...
	HGDIOBJ	oldBmp = ::SelectObject(hTmpDc, hClipBmp);
...
int TChildView::PaintBmpIcon(HBITMAP hBmp, TRect rc, BOOL is_direct)
{
	...
	::BitBlt(hDc, rc.left, rc.top, rc.cx(), rc.cy(), hTmpDc, 0, 0, SRCCOPY);
	::SelectObject(hTmpDc, oldBmp);

	if (is_direct) ::ReleaseDC(hWnd, hDc);
...
int TChildView::PaintFoldBtn(ViewMsg *msg, int top_y, int cnt, int body_btm)
{
	...
	if (IsRealDraw()) {
		...
		if (msg->unOpenR && bottom_y > body_btm) {
			PaintUnOpenBox(0, body_btm, msg->sz.cx, bottom_y - body_btm);
		}
		PaintRect(hMemDc, hGrayPen, hShainBrush, rc, 5, 5);
		...
		HGDIOBJ	svObj = ::SelectObject(hTmpDc, IsFold(msg) ? hFoldTBmp : hFoldFBmp);
...
{
	TRect	rc(0, top_y, vrc.cx(), headCy);

	PaintRect(hMemDc, hGrayPen, hGrayBrush, rc);
	PaintPushBox(rc, NORMAL_BOX);
...
static int PaintThumbCore(int cx, int cy, int clip_x, int clip_y, int width,
...
int TChildView::PaintThumb(ViewMsg *msg, int top_y, int *_width, BOOL is_direct)
{
	...
	for (int i=0; i < msg->msg.clip.size(); i++) {
		...
		if (IsRealDraw()) {
			...
			PaintThumbCore(img.GetWidth(), img.GetHeight(), clip_x, clip_y, width,
			...
			PaintThumbCore(clip.sz.cx, clip.sz.cy, clip_x, clip_y, width,
				&rc, &top_y, &diff_x_min);
		}
		clip_y += rc.Height + 1;
...
int TChildView::PaintMsg(ViewMsg *msg, int top_y)
{
...
#define FOCUS_YOFF 1
	if (ViewMsg *msg = GetMsg(msgIds[curIdx].msg_id)) {
		TRect	rc(1, msg->dispYOff + FOCUS_YOFF, FOCUS_WIDTH, headCy - FOCUS_YOFF*2);
...
BOOL TChildView::CalcMaxLineChars()
{
	...
	if (!test_data) {
		test_data = new char [MAX_LINECHARS + 1];
		memset(test_data, ' ', MAX_LINECHARS);
		test_data[MAX_LINECHARS] = 0;
	}
