...
/*
	TMsgBoxは位置決め可能な MessageBox()、Modal用
*/
...
/*
	Window生成ルーチン
*/
int TMsgBox::Exec(const char *_text, const char *_title, DWORD _flags, int _x, int _y)
{
	text  = _text;
	title = _title;
	flags = _flags;
...
int TMsgBox::Exec(const char *_text, const char *_title, DWORD _flags, BOOL is_cursor)
{
	int	tmp_x = CW_USEDEFAULT;
	int	tmp_y = CW_USEDEFAULT;
	...
	return	Exec(_text, _title, _flags, tmp_x, tmp_y);
}
