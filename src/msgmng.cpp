...
ULONG MsgMng::InitIPDict(IPDict *ipdict, ULONG cmd, ULONG flags, ULONG _pktno)
{
	...
	ipdict->put_int(IPMSG_DATE_KEY, Time());
...
BOOL MsgMng::SignIPDict(IPDict *ipdict)
{
	...
	if (::CryptCreateHash(csp, CALG_SHA_256, 0, 0, &hHash)) {
		if (::CryptHashData(hHash, dbuf.Buf(), (DWORD)size, 0)) {
			...
			if (!::CryptSignHash(hHash, AT_KEYEXCHANGE, 0, 0, dbuf.Buf(), &sigLen)) {
				sigLen = 0;
			}
		}
...
BOOL MsgMng::EncIPDict(IPDict *ipdict, PubKey *pub, IPDict *out_dict)
{
	...
	if (!::CryptEncrypt(hExKey, 0, TRUE, 0, key, &enc_keylen, MAX_BUF)) {
...
{
	const DWORD	enc_key_len = 2048 / 8;
	DynBuf	iv;
	DynBuf	key;
	DynBuf	enc_body;
...
BOOL MsgMng::CheckVerify(MsgBuf *msg, const IPDict *ipdict)
{
	...
	HCRYPTPROV	verify_csp = cfg->priv[KEY_2048].hPubCsp;
	...
	host->pubKey.KeyBlob(blob, sizeof(blob), &bloblen);	// KeyBlob 作成/import

	if (!::CryptImportKey(verify_csp, blob, bloblen, 0, 0, &hExKey)) {
		return	ret;
	}
	if (::CryptCreateHash(verify_csp, CALG_SHA_256, 0, 0, &hHash)) {
	...
	BYTE *out, size_t max_len)
{
	...
	for (auto i=masterPub.begin(); i != masterPub.end(); i++) {
		int		blob_len;
		BYTE	blob[MAX_BUF];
		// KeyBlob 作成
		(*i)->KeyBlob(blob, sizeof(blob), &blob_len);
...
void MsgMng::UListToHostSubDict(MsgBuf *msg, vector<HostSub> *hosts)
{
	...
	for_each(dlist.begin(), dlist.end(), [&](auto &h) {
		HostSub	hostSub;
		U8str	uid;
		U8str	hid;
...
void MsgMng::UserToUList(const vector<User> &uvec, char *ulist)
{
	...
	for (auto itr=uvec.begin(); itr != uvec.end(); itr++) {
		if (len + MAX_NAMEBUF * 2 + 3 >= MAX_ULISTBUF) {
			break;
		}
		len += sprintf(ulist+len, "%s:%s%c", itr->userName, itr->hostName, HOSTLIST_SEPARATOR);
	}
}
...
void MsgMng::UserToUListDict(const vector<User> &uvec, IPDict *dict)
{
	...
	for (auto itr=uvec.begin(); itr != uvec.end(); itr++) {
		...
		h->put_str(IPMSG_UID_KEY,  itr->userName);
		h->put_str(IPMSG_HOST_KEY, itr->hostName);
		dlist.push_back(h);
	}
