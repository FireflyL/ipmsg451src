...
BOOL LogDb::AddTblDataAdd(AddTblData *atd, LogMsg *m, int64 explicit_msgid)
{
	...
	for (auto itr=m->clip.begin(); itr != m->clip.end(); itr++) {
		int	cidx = 1;
		sqlite3_bind_int64(atd->ins_clip,  cidx++, msg_id);
		sqlite3_bind_text16(atd->ins_clip, cidx++, itr->fname.s(), -1, SQLITE_STATIC);
		sqlite3_bind_int(atd->ins_clip,  cidx++, itr->sz.cx);
		sqlite3_bind_int(atd->ins_clip,  cidx++, itr->sz.cy);
		ret = sqlite3_step(atd->ins_clip);
		sqlite3_reset(atd->ins_clip);
		if (ret != SQLITE_DONE) goto ERR;
	}
	for (auto itr=m->files.begin(); itr != m->files.end(); itr++) {
		int	fidx = 1;
		sqlite3_bind_int64(atd->ins_file,  fidx++, msg_id);
		sqlite3_bind_text16(atd->ins_file, fidx++, itr->s(), -1, SQLITE_STATIC);
		ret = sqlite3_step(atd->ins_file);
		sqlite3_reset(atd->ins_file);
		if (ret != SQLITE_DONE) goto ERR;
	}
	for (auto itr=m->mark.begin(); itr != m->mark.end(); itr++) {
		int	midx = 1;
		sqlite3_bind_int64(atd->ins_mark, midx++, msg_id);
		sqlite3_bind_int(atd->ins_mark,   midx++, itr->kind);
		sqlite3_bind_int(atd->ins_mark,   midx++, itr->pos);
		sqlite3_bind_int(atd->ins_mark,   midx++, itr->len);
