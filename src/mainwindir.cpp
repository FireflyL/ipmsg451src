	...
	for (auto itr=dlist.begin(); itr != dlist.end(); itr++) {
		...
		if (host.active) {
			...
			if (IsSameHost(&host.hostSub, msgMng->GetLocalHost())) {
				if (lastExitTick && (GetTickCount() - lastExitTick) > 5000) {
...
BOOL TMainWin::AgentDirHost(Host *host, BOOL is_direct)
{
	...
	if (::GetTickCount() - brDirAgentLast > brDirAgentLimit) {
		return FALSE;
	}
	if (!host->hostSub.addr.IsInNet(brDirAddr)) {
		return FALSE;
...
{
	for_each(segMap.begin(), segMap.end(), [&](auto &itr) {
...
//	Debug("DirSendAgentBroad\n");
	...
	for (auto s=segMap.begin(); s != segMap.end(); s++) {
		auto	&seg_h   = s->second;
		...
		for (auto itr=agents.begin(); itr != agents.end(); itr++) {
			if (itr->isReq || (!need_br && (cur - itr->resTick < loop_tick && !force_agent))) {
				continue;
			}
			Host	*host = itr->host;
...
void TMainWin::DirClean()
{
	...
	for (auto i=lru.rbegin(); i != lru.rend(); i++) {
		...
		if (diff < cfg->dirSpan + DIR_BASE_SEC * 8) {
			...
			host->RefCnt(1);
			DirDelHost(host, FALSE);
			hlist.push_back(host);
		}
	}
	if (hlist.size() > 0) {
		DirSendAnsHosts(hlist);
		for_each(hlist.begin(), hlist.end(), [&](auto &h) {
			h->RefCnt(-1);
			h->SafeRelease();
		});
	}
...
{
//	Debug("DirRefreshAgent segs=%zd ...", segMap.size());
	DWORD	cur = ::GetTickCount();
...
BOOL DetectPollNet(MsgBuf *msg, Addr *addr)
{
	...
	for (auto i=bl.begin(); i != bl.end(); i++) {
		U8str	s;
		int64	mask;
		if (!(*i)->get_str(IPMSG_ADDR_KEY, &s)) continue;
		if (!(*i)->get_int(IPMSG_MASK_KEY, &mask) || mask < 0 || mask > 32) continue;
...
Host *TMainWin::DirAddHost(Host *host, BOOL is_poll, BOOL is_notify, BOOL *_need_notify)
{
	...
	if (Host *nh = allHosts.GetHostByName(&host->hostSub)) {
		if (IsSameAddrPort(&nh->hostSub, &host->hostSub)) {
			...
			nh->updateTime = nh->updateTimeDirect = Time();
...
//				Debug("DirAddHost no update %s/%d poll=%d\n", nh->hostSub.addr.S(), nh->hostSub.addr.mask, is_poll);
	...
	Host *dh = new Host;
	*dh = *host;
	dh->updateTime = dh->updateTimeDirect = Time();
...
BOOL TMainWin::TcpDirEvent(SOCKET sd, LPARAM lParam)
{
	...
	pkt->info.lastTick = ::GetTickCount();
...
BOOL TMainWin::CleanupDirTcp()
{
	...
	for_each(sdvec.begin(), sdvec.end(), [&](auto sd) {
