...
BOOL TChildView::SetStatusMsg(int pos, const WCHAR *msg, DWORD duration, BOOL use_stat)
{
	...
	DWORD cur = (lastTick || duration) ? ::GetTickCount() : 0;
...
void AddMarkWithCompaction(vector<LogMark> *_log_vec, int pos, int len)
{
	...
	if (!done) {
		auto itr = log_vec.begin();
		for ( ; itr != log_vec.end(); itr++) {
			if (pos < itr->pos) {
				break;
			}
		}
...
BOOL TChildView::IsUnOpenCore(LogMsg *msg, bool is_receive, int idx)
{
	if (msg->msg_id >= beginMsgId &&
		...
		for (auto itr=msg->host.begin(); itr != msg->host.end(); itr++) {
			if ((itr->flags & DB_FLAGMH_UNOPEN)) {
				return	TRUE;
			}
		}
	}
...
int TChildView::AnalyzeUrlMsgCore(std::vector<BodyAttr> *link, const WCHAR *s)
{
	...
	for (int i=0; s[i]; i=attr.pos+attr.len) {
		...
		attr.pos = i + (int)match.position(0);
		attr.len =     (int)match.length(0);
...
BOOL TChildView::ReplyMsg(ViewMsg *msg, int idx)
{
	...
	if (IsSelected(TRUE)) {
		...
		body.Init(msg->msg.body.s() + pos, len);
	}
	else {
		body.Init(msg->msg.body.s());
	}
...
BOOL TChildView::DelMsg(int64 msg_id)
{
	...
	if (TMsgBox(this).Exec(LoadStrU8(IDS_DELMSG), IP_MSG, MB_OKCANCEL) != IDOK) {
		return	FALSE;
	}
...
{
	if (TMsgBox(this).Exec(Fmt(LoadStrU8(IDS_DELALLMSG_FMT), end_idx - top_idx + 1),
...
{
	if (TMsgBox(this).Exec(LoadStrU8(IDS_DELCLIPFULL), IP_MSG, MB_OKCANCEL) != IDOK) {
		return	FALSE;
	}
