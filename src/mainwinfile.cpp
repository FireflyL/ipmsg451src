	...
	if (obj->fileInfo->MemData()) {
		...
		if (obj->isDir) {
			...
			if ((cfg->fileTransOpt & FT_STRICTDATE) &&
				*(int64 *)&obj->fdata.ftLastWriteTime > *(int64 *)&obj->attachTime) {
...
int MakeDirHeader(SendFileObj *obj, BOOL find)
{
	...
	if (find) {
		len = sprintf(obj->header, "0000:%s:%x%08x:%x:%x=%x:%x=%x:", cFileName,
		...
		len = sprintf(obj->header, "0000:.:0:%x:%x=%x:%x=%x:", ipmsg_attr,
				IPMSG_FILE_MTIME, FileTime2UnixTime(&dat->ftLastWriteTime),
				IPMSG_FILE_CREATETIME, FileTime2UnixTime(&dat->ftCreationTime));
	}
	else {
	...
	obj->header[sprintf(obj->header, "%04x", len)] = ':';
