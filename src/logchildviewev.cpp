static char *logchildviewev_id = 
	"@(#)Copyright (C) H.Shirouzu and Asahi Net, Inc. 2015-2017   logchildviewev.cpp Ver4.50";
/* ========================================================================
...
BOOL TChildView::EvCreate(LPARAM lParam)
{
	...
	findBodyVec.clear();
	findUser.Init();
	dispFlags = (cfg->logViewDispMode & 1) ? DISP_REV : 0;
	foldGenNo++;
...
//	SelectObject(hTmpDc, ::CreateCompatibleBitmap(hDesktopDc, 12, 12));
	...
	hShainPen	= ::CreatePen(PS_SOLID, 1, SHAIN_RGB);
	...
	lb.lbColor = SHAIN_RGB;
	hShainBrush	= ::CreateBrushIndirect(&lb);
...
//				DebugW(L"combo=%s old=%s\n", buf.s(), findBodyOrg.s());
	...
	case MENU_DELCOMMENT:
		if (itemMenu && itemMenu->msg_id) {
			...
			if (TMsgBox(this).Exec(LoadStrU8(IDS_DELCOMMENT), IP_MSG, MB_OKCANCEL) == IDOK) {
				Wstr	empty_comment;
...
BOOL TChildView::EvTimer(WPARAM timerID, TIMERPROC proc)
{
	switch (timerID) {
	case FIND_TIMER:
		if (findTick && (GetTickCount() - findTick) > FIND_TICK) {
	...
	case FINDNEXT_TIMER:
		if (scrHresReduce && (GetTickCount() - scrHresReduce) > FINDNEXT_TICK) {
			scrHresReduce = 0;
			KillTimer(FIND_TIMER);
			SetScrollPos();
		}
...
#define SEL_NONINIT (Item::FROMTO|Item::CLIP|Item::CLIPBODY|Item::FAV|Item::IMENU)
	...
	if ((item->kind & (Item::UNOPEN|Item::UNOPENBODY)) ||
		...
		if (msg) {
			msg->foldGenNo = IsFold(msg) ? 0 : foldGenNo;
			if (!IsFold(msg)) {
...
BOOL TChildView::ButtonUpItem(Item *item, POINTS pos)
{
	...
	if (item->kind & (Item::HEADBOX | Item::BODYBLANK)) {
		if (dblClickStart && (GetTickCount() - dblClickStart) < 500) {
...
BOOL TChildView::ButtonDblClickItem(Item *item, POINTS pos)
{
	...
	if (item->kind &
		...
		if ((dblClickStart = ::GetTickCount()) == 0) {
		...
		if (ViewMsg *msg = GetMsg(last_item->msg_id)) {
			if (last_item->kind & Item::FROMTO) {
				...
				if (msg->msg.flags & DB_FLAG_FROM) {
					...
					irc.Inflate(1, 1);
					PaintPushBox(irc, CLEAR_BOX, TRUE);
				}
			}
		...
		if (item->kind & Item::FROMTO) {
			...
			if (msg) {
				if (msg->msg.IsRecv()) {
					...
					irc.Inflate(1, 1);
					PaintPushBox(irc, NORMAL_BOX, TRUE);
					::SetCursor(hHandCur);
				}
			}
		}
