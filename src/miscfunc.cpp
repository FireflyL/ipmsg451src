static char *miscfunc_id = 
	"@(#)Copyright (C) H.Shirouzu 2011-2017   miscfunc.cpp	Ver4.50";
/* ========================================================================
...
LinkKind GetLinkKind(Cfg *cfg, const WCHAR *s)
{
	...
	if (regex_search(s, match, *cfg->file_re)) {
		return	LK_FILE;
	}

	if (regex_search(s, match, *cfg->fileurl_re)) {
		return	LK_FILE_URL;
	}

	if (regex_search(s, match, *cfg->url_re)) {
		return	LK_URL;
	}
...
BOOL CheckExtW(Cfg *cfg, const WCHAR *s, BOOL *is_dir)
{
	...
	*ext = 0;
	...
	for (auto itr=cfg->directExtVec.begin(); itr != cfg->directExtVec.end(); itr++) {
		if (wcsicmp(itr->s(), ext) == 0) {
			return	TRUE;
		}
	}
	return	FALSE;
...
JumpLinkRes JumpLinkWithCheck(TWin *parent, Cfg *cfg, const WCHAR *s, DWORD flg)
{
	...
	if (lk == LK_FILE || lk == LK_FILE_URL) {
		if (cfg->linkDetectMode & 1) {	// ファイル系は実行禁止
			TMsgBox(parent).Exec( Fmt("%s", LoadStrU8(IDS_DISABLEFILELINK)), IP_MSG, MB_OK);
			return	JLR_SKIP;
		}
		if (!CheckExtW(cfg, s)) {
			if (TMsgBox(parent).Exec( Fmt("%s\r\n%s", LoadStrU8(IDS_ALERTFILELINK), WtoU8s(s)),
				IP_MSG, MB_OKCANCEL, TRUE) != IDOK) {
				return	JLR_SKIP;
			}
		}
	}
...
int MakeSizeString(char *buf, int64 size, int flg)
{
	...
	{
		if (flg & MSS_NOPOINT)
			return	sprintf(buf, "%d%sMB", (int)(size / 1024 / 1024), flg & MSS_SPACE ? " " : "");
		else
			return	sprintf(buf, "%d.%d%sMB", (int)(size / 1024 / 1024), (int)((size * 10 / 1024 / 1024) % 10), flg & MSS_SPACE ? " " : "");
	}
	else return	sprintf(buf, "%d%sKB", (int)(ALIGN_BLOCK(size, 1024)), flg & MSS_SPACE ? " " : "");
}
...
BOOL MakeDownloadLinkDirW(Cfg *cfg, Time_t _t, WCHAR *dir)
{
...
#define DLLINK_MAX	(MAX_PATH - 80)
	...
	swprintf(link + len, L"_%x_%c.lnk", t, is_recv ? 'r' : 's');
	return	TRUE;
}
