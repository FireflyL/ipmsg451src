static char *mainwinev_id = 
	"@(#)Copyright (C) H.Shirouzu 1996-2017   mainwinev.cpp	Ver4.50";
/* ========================================================================
...
/*
	TRegisterClassU8("ipmsg_test", CS_DBLCLKS, 0, 0, (HBRUSH)COLOR_WINDOW);
	
	TWin	*win = new TWin();
	win->CreateU8("ipmsg_test", "test", WS_OVERLAPPEDWINDOW|WS_CLIPCHILDREN|WS_VISIBLE);
	win->Show(SW_SHOW);
	win->MoveWindow(100, 100, 100, 100, TRUE);
	::CreateWindowW(L"EDIT", L"", WS_CHILD|WS_VISIBLE, 0, 0, 100, 100, win->hWnd, 0, 0, 0);
*/
...
//			DeleteJumpList();
	...
	if (cfg->viewEpoch) {
		if (auto view=logViewList.TopObj()) {
			view->SetStatusCheckEpoch((time_t)cfg->viewEpoch);
		}
	}
...
#undef  MAIN_EVCREATE_2
	...
	if (!*cfg->NickNameStr) {
		BalloonWindow(TRAY_NORMAL, LoadStrU8(IDS_FIRSTRUN_TRAY), "IPMSG", 60000000, TRUE);
	}
